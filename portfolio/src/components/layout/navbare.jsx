...
import TerminalMode from '../effects/TerminalMode';

export default function Navbar() {
  // 1. Déclaration unique de tous les états (Pas de doublons)
  const [theme, setTheme] = useState('dark');
  const [language, setLanguage] = useState('FR');
  const [activeLink, setActiveLink] = useState('#about');
  const [isTerminalOpen, setIsTerminalOpen] = useState(false);

  // Liens de navigation dynamiques selon la langue sélectionnés
  const navLinks = {
    FR: [
  ...
  return (
    <>
      <div className="w-full fixed top-0 left-0 p-4 z-50 flex justify-center font-mono">
        <nav className={`w-full max-w-6xl backdrop-blur-md border rounded-xl shadow-xl overflow-hidden transition-colors duration-300 ${
          ...
          <div className="px-5 py-3 flex items-center justify-between gap-4">
            ...
            {/* Boutons de contrôle (Terminal, Thème, Langue, Contact) */}
            <div className="flex items-center gap-3 text-xs">
              
              {/* BOUTON DU MODE TERMINAL INTERACTIF */}
              <button 
                onClick={() => setIsTerminalOpen(true)}
                className="px-2.5 py-1.5 border border-green-500/40 text-green-400 bg-green-950/10 rounded font-bold hover:bg-green-500/20 transition-all active:scale-95 cursor-pointer"
              ...
              <button 
                onClick={toggleTheme}
                className={`px-2 py-1.5 rounded border font-bold transition-all active:scale-95 cursor-pointer ${
                  theme === 'night' 
                    ? 'border-green-500/30 text-green-400 hover:bg-green-500/10' 
                    : 'border-zinc-700 text-yellow-500 hover:bg-zinc-800'
                }`}
      ...
      {/* Rendu du terminal interactif en arrière-plan (S'affiche uniquement si isTerminalOpen est true) */}
